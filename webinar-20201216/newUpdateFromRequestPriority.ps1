...
function Write-CustomLog {
    ...
    Param (
        ...
        [Parameter()]
        [string]$fileName = "logs\newUpdateFromRequestPriority_$today.log",
...
try {
    Write-CustomLog -Message "Sending update to Microsoft Teams"
    Send-MessageToTeams -Title "Ny prioritet" -TitleText "$($easitObjects.itemtype) #$($easitObjects.id)" -ActivityTitle "Status: $($easitObjects.status) - Prioritet: $($easitObjects.priority)" -ActivitySubtitle "$($easitObjects.subject)" -ActivityText "$($easitObjects.description)"
    Write-CustomLog -Message "Update sent to Microsoft Teams"
} catch {
