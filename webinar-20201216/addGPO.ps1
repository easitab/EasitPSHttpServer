...
function Write-CustomLog {
    ...
    Param (
        ...
        [Parameter()]
        [string]$fileName = "logs\addGPO_$today.log",
...
if ("$($easitObjects.type)") {
    try {
        ...
        New-GPStarterGPO @gpParameters -ErrorAction Stop
    ...
    if ("$($easitObjects.starterGPOName)") {
        $gpParameters.Add('StarterGPOName',"$($easitObjects.starterGPOName)")
    }
    if ("$($easitObjects.targetId)") {
        try {
            New-GPO @gpParameters -ErrorAction Stop | new-gplink -target "$($easitObjects.targetId)" -ErrorAction Stop
        } catch {
            Write-CustomLog -InputObject $_ -Level ERROR
        }
    } else {
        try {
            New-GPO @gpParameters -ErrorAction Stop
        } catch {
            Write-CustomLog -InputObject $_ -Level ERROR
        }
    }
...
if ("$($easitObjects.type)") {
    $result = Get-GPStarterGPO -Name "$($easitObjects.name)"
} else {
    $result = Get-GPO -Name "$($easitObjects.name)"
}
