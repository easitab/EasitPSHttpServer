...
try {
    $adUpdate = Activate-ADAccount -Identity "$($easitObjects.userName)"
} catch {
...
if ($adUpdate) {
    try {
        $easitContactUpdate = Import-GOContactItem -url "$urlWS" -apikey "$apikey" -ImportHandlerIdentifier "$importHandlerContact" -Inactive $false
    } catch {
        "$_" | Out-File -Path "$outputFile" -Encoding UTF8 -Append
        break
    }
